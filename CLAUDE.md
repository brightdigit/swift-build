...
The action can parse test output and extract test statistics to validate test counts and provide structured test data.

### Inputs

- `parse-test-results`: Enable parsing (default: false)
- `expected-test-count`: Validate against expected count (optional, fails if mismatch)
- `fail-on-parse-error`: Fail on parsing errors (default: false, validation failures always fail)

### Outputs

- `test-total`: Total tests executed
- `test-passed`: Tests that passed
- `test-failed`: Tests that failed
- `test-skipped`: Tests that were skipped (Swift Testing only)
- `test-parsing-status`: Status indicator (success, failed, disabled, not-applicable, validation-failed)
...
- uses: YourOrg/swift-build@v2
  ...
  with:
    scheme: MyPackage
    parse-test-results: true
    expected-test-count: 42
    fail-on-parse-error: true

- name: Display Test Results
  if: steps.build.outputs.parsing-status == 'success'
  run: |
    echo "Total: ${{ steps.build.outputs.test-total }}"
    echo "Passed: ${{ steps.build.outputs.test-passed }}"
    echo "Failed: ${{ steps.build.outputs.test-failed }}"
    echo "Skipped: ${{ steps.build.outputs.test-skipped }}"
```

### Platform Support

- ✓ **SwiftPM** (macOS, Ubuntu, Windows) - Auto-detects Swift Testing vs XCTest, handles mixed frameworks
- ✓ **Xcode** (all Apple platforms) - Parses xcodebuild and xcbeautify output
- ✓ **Wasm** (including "both" mode) - Sums Swift Testing + XCTest when both present
- ✗ **Android** - Not applicable (output not capturable from external action)

### Test Count Validation

Use `expected-test-count` to ensure tests aren't accidentally skipped:
...
- uses: YourOrg/swift-build@v2
  with:
    scheme: MyPackage
    parse-test-results: true
    expected-test-count: 17  # Fails if actual count ≠ 17
```

Validation failures always fail the workflow regardless of `fail-on-parse-error` setting. Useful for detecting:
- Configuration issues that skip tests
- Test framework detection problems
- Missing test targets

### Notes

- **Text-based parsing**: Uses regex patterns on test output (most consistent across platforms)
- **Mixed frameworks**: Automatically detects and sums Swift Testing + XCTest test counts
- **Wasm "both" mode**: Correctly sums tests from two separate test runs
- **Android limitation**: Tests executed by external action, output not capturable via composite action `uses:` steps
