...
| `android-sdk-url` | Custom Android Swift SDK artifactbundle URL | `null` | `https://download.swift.org/swift-6.4.x-branch/android-sdk/.../<snapshot>_android.artifactbundle.tar.gz` | Any artifactbundle URL | **Android builds only** - Passed to `custom-sdk-url` in [skiptools/swift-android-action](https://github.com/skiptools/swift-android-action). Requires `android-sdk-id`. Use to target a nightly/snapshot Android SDK the `android-swift-version` path can't resolve |
| `android-sdk-id` | Identifier for the custom Android SDK | `null` | `swift-6.4.x-DEVELOPMENT-SNAPSHOT-2026-06-15-a_android` | The `swift sdk` id the bundle registers under | **Android builds only** - Maps to `custom-sdk-id`. Required alongside `android-sdk-url`. Must match the id shown by `swift sdk list` after the bundle installs (a common source of misconfiguration) |
| `android-sdk-target` | Swift SDK build triple for the custom Android SDK | `aarch64-unknown-linux-android28` (when `android-sdk-url` set) | `x86_64-unknown-linux-android28` | Any registered SDK triple | **Android builds only** - Maps to `installed-sdk` (the `--swift-sdk <triple>` target). swift-android-action **requires** a non-empty value alongside a custom SDK or it exits 1. Override if the bundle registers a different triple (check `swift sdk configure --show-configuration`) |
...
| `wasm-sdk-url` | Custom Wasm SDK artifactbundle URL | `null` | `https://download.swift.org/swift-6.4.x-branch/wasm-sdk/.../<snapshot>_wasm.artifactbundle.tar.gz` | Any artifactbundle URL | **Wasm builds only** - Overrides the derived `download.swift.org` `-RELEASE` URL; the `--swift-sdk` selector is derived from the bundle filename. Use for nightly/snapshot toolchains (no `-RELEASE` Wasm bundle exists). Pair with `wasm-sdk-checksum` |
| `wasm-sdk-checksum` | SHA256 checksum for `wasm-sdk-url` | `null` | `c014a0162de6...` | SHA256 hex string | **Wasm builds only** - Passed to `swift sdk install --checksum` for verified installation. Recommended whenever `wasm-sdk-url` is set |
...
#### Android with Test Resources
```yaml
name: Android with Test Resources
on: [push, pull_request]
    ...
    steps:
      ...
      - uses: brightdigit/swift-build@v1
        with:
          ...
          android-swift-version: "6.2"
          android-api-level: "28"
          android-copy-files: "Tests/Resources/ TestData/fixtures.json"
```

> **Note:** The `android-copy-files` parameter copies specified files and directories to the Android emulator before running tests. This is useful when your tests need access to test fixtures, mock data, configuration files, or other resources at runtime. Specify multiple files/directories separated by spaces.
